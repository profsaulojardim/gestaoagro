/* Service Worker do "Gestão do Rebanho"
   V92: atualização previsível no iPhone sem cachear Supabase. */

const CACHE = "rebanho-v92";
const APP_VERSION = "92";
const CORE = ["./manifest.json", "./icon.png", "./icon-192.png", "./icon-512.png", "./icon-maskable-512.png", "./boi.png", "./bezerro.png", "./troca-v81.js", "./troca-v81-core.js", "./perfil-backup-v87.js", "./perfil-cleanup-v90.js", "./ui-cleanup-v91.js", "./saida-v92.js"];
...
async function paginaAtual(request){
  const res=await fetch(request,{cache:"no-store"});if(!res.ok)return res;const tipo=res.headers.get("content-type")||"";if(!tipo.includes("text/html"))return res;let html=await res.text();
  const scripts=["troca-v81.js","perfil-backup-v87.js","perfil-cleanup-v90.js","ui-cleanup-v91.js","saida-v92.js"];
  for(const arq of scripts){const re=new RegExp(`<script\\s+src=["']${arq.replace('.', '\\.')}(?:\\?[^"']*)?["']><\\/script>`,`gi`);html=html.replace(re,'');}
  html=html.replace(/<\/body>/i,`${atualizadorInline()}<script src="troca-v81.js?v=${APP_VERSION}"></script><script src="perfil-backup-v87.js?v=${APP_VERSION}"></script><script src="perfil-cleanup-v90.js?v=${APP_VERSION}"></script><script src="ui-cleanup-v91.js?v=${APP_VERSION}"></script><script src="saida-v92.js?v=${APP_VERSION}"></script></body>`);
  const headers=new Headers(res.headers);headers.delete("content-length");headers.delete("content-encoding");headers.set("cache-control","no-store");const out=new Response(html,{status:res.status,statusText:res.statusText,headers});const c=await caches.open(CACHE);await c.put("./index.html",out.clone());return out;
}
