/* Service Worker do "Gestão do Rebanho"
   V86: atualização previsível no iPhone sem cachear Supabase. */

const CACHE = "rebanho-v86";
  ...
  e.waitUntil((async()=>{
    ...
    for(const c of cs){
      try{
        ...
        if(u.origin===self.location.origin){u.searchParams.set("appv",APP_VERSION);await c.navigate(u.href);}
      }catch(_){ }
    }
  })());
});
...
function atualizadorInline(){
  return `<script>(function(){
    ...
    setInterval(function(){if(document.visibilityState==='visible')checar();},30000);
  })();<\/script>`;
}
